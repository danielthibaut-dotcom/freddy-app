...
def write_musicxml(model: Dict[str, Any], path: Path) -> None:
    ...
    by_measure_staff: Dict[Tuple[int, str], List[Dict[str, Any]]] = defaultdict(list)
    for n in notes:
        by_measure_staff[(n["measure"], n["staff"])].append(n)
    parts = [("P1", "Treble", "treble"), ("P2", "Bass", "bass")]
    ...
    for pid, _name, staff in parts:
        ...
        for mnum in sorted(measures):
            ...
            if mnum == first_measure or sig != last_sig:
                ...
                lines.append(f'        <clef><sign>{"G" if staff == "treble" else "F"}</sign><line>{"2" if staff == "treble" else "4"}</line></clef>')
                lines.append('      </attributes>')
                last_sig = sig
            grouped: Dict[int, List[Dict[str, Any]]] = defaultdict(list)
            for n in sorted(by_measure_staff.get((mnum, staff), []), key=lambda x: (x["q_start_tick"], x["midi_note"])):
...
def render_pdf(model: Dict[str, Any], path: Path):
    ...
    for n in notes: notes_by_measure[int(n['measure'])].append(n)
    tempo = model.get('tempo_map', [{}])[0].get('bpm', '-') if model.get('tempo_map') else '-'
    sha = str(model.get('source', {}).get('sha256', ''))[:12]
    margin_x = 42; top = 66; bottom = 40; spacing = 6.2
    usable_w = A4_W - margin_x*2
    clef_w = 28; staff_x = margin_x + clef_w
    system_h = 118; systems_per_page = 6; measures_per_system = 4
    measure_w = (usable_w - clef_w) / measures_per_system
    def page_header(page_no:int):
        ...
        pdf.text(A4_W/2, A4_H-62, f'Tempo: {tempo} bpm | Track: {model.get("filters",{}).get("chosen_track")} | SHA: {sha}', 7.5, False, 'center')
        pdf.text(A4_W-margin_x, 22, f'Page {page_no} | Generated by FREDDYnote', 7, False, 'right')
    ...
    for idx in range(0, len(measures), measures_per_system):
        ...
        draw_staff(pdf, staff_x, treble_bottom, usable_w-clef_w, spacing)
        draw_staff(pdf, staff_x, bass_bottom, usable_w-clef_w, spacing)
